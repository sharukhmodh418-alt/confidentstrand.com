...
const bookingSchema = new mongoose.Schema({
    id: { type: String, required: true, unique: true },
    bookingType: { type: String, default: 'Maintenance', enum: ['Maintenance', 'Direct Booking', 'Partner', 'Consultation'] },
    name: { type: String, required: true },
    phone: { type: String, required: true },
...
function normalizeTimeTo24h(timeStr) {
    ...
    // 12-hour format: e.g. "04:00 PM", "4:00 PM", "4:00PM", "4 PM", "4PM"
    ...
    // 24-hour format: e.g. "16:00", "16:00:00", "04:00"
...
// Availability / Booked Slots Endpoint (Safe for public frontend)
app.get(['/api/bookings/booked-slots', '/api/bookings/availability', '/api/availability'], async (req, res) => {
    try {
        ...
        if (date) {
            const dateBookings = activeBookings.filter(b => b.date === date);
            const bookedSlots24 = dateBookings.map(b => normalizeTimeTo24h(b.time)).filter(Boolean);
            const bookedRaw = dateBookings.map(b => b.time).filter(Boolean);
            
            return res.status(200).json({
                success: true,
                date,
                bookedSlots: bookedRaw,
                bookedSlots24: [...new Set(bookedSlots24)]
            });
        }

        // If no date specified, return booked dates and slots mapping
        const dateSlotMap = {};
        activeBookings.forEach(b => {
            if (!b.date || !b.time) return;
            if (!dateSlotMap[b.date]) dateSlotMap[b.date] = [];
            const t24 = normalizeTimeTo24h(b.time);
            if (t24 && !dateSlotMap[b.date].includes(t24)) {
                dateSlotMap[b.date].push(t24);
            }
        });

        res.status(200).json({
            success: true,
            bookedByDate: dateSlotMap
        });
    } catch (err) {
...
app.post('/api/bookings', async (req, res) => {
    const { name, phone, date, time, service, notes, status, bookingType } = req.body;
    ...
    try {
        const allBookings = await getBookings();
        const reqTime24 = normalizeTimeTo24h(time);
        const isConflict = (allBookings || []).some(b => 
            b.date === date && 
            b.status !== 'Cancelled' && 
            normalizeTimeTo24h(b.time) === reqTime24
        );

        if (isConflict) {
            return res.status(409).json({
                success: false,
                error: `The ${time} slot on ${date} is already booked. Please choose another slot.`
            });
        }
    } catch (checkErr) {
        console.warn('Slot conflict check warning (proceeding):', checkErr);
    }

    // Determine booking type if not explicitly supplied
    let resolvedType = bookingType;
    if (!resolvedType) {
        if (req.body.stylist) {
...
function getArtistShortId(name) {
    if (!name) return '';
    const firstWord = name.trim().split(/[\s\-]+/)[0].toLowerCase();
    return firstWord || slugifyArtist(name);
}

// Get all artists with their unique portal link identifiers
app.get('/api/artists', async (req, res) => {
    try {
        const configs = await getAllPartnerConfigs();
        const artists = configs.map(c => {
            const slug = slugifyArtist(c.stylist);
            const shortId = getArtistShortId(c.stylist);
            return {
                name: c.stylist,
                slug: slug,
                shortId: shortId,
                salonShare: c.salonShare || 70,
                services: c.services || [],
                uniquePath: `/artist.html?id=${slug}`,
                shortPath: `/artist.html?id=${shortId}`
            };
        });
...
// Dedicated Artist Portal Data Endpoint
app.get('/api/artist/portal', async (req, res) => {
    const { id, slug, stylist, token } = req.query;
    const queryIdentifier = (id || slug || stylist || token || '').trim().toLowerCase();

    if (!queryIdentifier) {
        return res.status(400).json({ success: false, error: 'Artist identifier is required.' });
    }

    try {
        const configs = await getAllPartnerConfigs();
        
        // Match by exact name, slug, shortId, or substring
        let matchedConfig = configs.find(c => {
            const artistName = c.stylist.toLowerCase();
            const artistSlug = slugifyArtist(c.stylist);
            const shortId = getArtistShortId(c.stylist);
            return artistName === queryIdentifier || 
                   artistSlug === queryIdentifier || 
                   shortId === queryIdentifier ||
                   artistName.includes(queryIdentifier);
        });

        if (!matchedConfig) {
            return res.status(404).json({ success: false, error: 'Artist profile not found for the provided link.' });
        }

        const canonicalName = matchedConfig.stylist;
        const allBookings = await getBookings();
        
        // Filter bookings assigned to this artist (case-insensitive and trimmed)
        const artistBookings = (allBookings || []).filter(b => {
            if (!b.stylist) return false;
            const bStylist = b.stylist.trim().toLowerCase();
            const cName = canonicalName.trim().toLowerCase();
            return bStylist === cName || 
                   bStylist === slugifyArtist(canonicalName) ||
                   cName.includes(bStylist) ||
                   bStylist.includes(getArtistShortId(canonicalName));
        });

        // Current date in YYYY-MM-DD
        const todayStr = new Date().toISOString().split('T')[0];

        // Enrich bookings with helper flags
        const enrichedBookings = artistBookings.map(b => {
            const bObj = b.toObject ? b.toObject() : { ...b };
            bObj.isToday = (b.date === todayStr);
            bObj.isUpcoming = (b.date > todayStr);
            bObj.isPast = (b.date < todayStr);
            return bObj;
        });

        // Sort: Today first, then upcoming ascending by date and time, then past descending
        enrichedBookings.sort((a, b) => {
            const dateA = a.date || '2099-12-31';
            const dateB = b.date || '2099-12-31';
            if (a.isToday && !b.isToday) return -1;
            if (!a.isToday && b.isToday) return 1;
            if (a.isUpcoming && b.isUpcoming) return dateA.localeCompare(dateB);
        ...
        res.status(200).json({
            ...
            artist: {
                name: canonicalName,
                slug: slugifyArtist(canonicalName),
                shortId: getArtistShortId(canonicalName),
                salonShare: matchedConfig.salonShare || 70,
                services: matchedConfig.services || []
            },
            todayDate: todayStr,
        ...
        console.error('Error fetching artist portal data:', err);
        res.status(500).json({ success: false, error: 'Failed to load artist portal data.' });
    }
});

// Artist quick status update endpoint
app.patch('/api/artist/bookings/:id/status', async (req, res) => {
    const { id } = req.params;
    const { status, artist } = req.body;
    ...
    try {
        ...
        console.error('Error updating booking status from artist portal:', err);
        res.status(500).json({ success: false, error: 'Failed to update status.' });
    }
});
...
// Serve dedicated artist portal explicitly
