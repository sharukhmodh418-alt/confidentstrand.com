...
document.addEventListener('DOMContentLoaded', () => {
  ...
  populateGlobalArtistDropdowns();
});

/* ---------- Automated WhatsApp Booking Notification System ---------- */
function triggerWhatsAppBookingNotification(booking) {
  const { type, name, phone, service, date, time, stylist, price, notes } = booking;

  let waText = `✨ *CONFIDENT STRANDS — NEW APPOINTMENT BOOKING*\n\n`;
  waText += `📋 *Type:* ${type || 'Appointment'}\n`;
  waText += `👤 *Client Name:* ${name}\n`;
  waText += `📞 *Client Phone:* ${phone}\n`;
  if (service) waText += `✂️ *Service:* ${service}\n`;
  if (stylist) waText += `🎨 *Selected Specialist:* ${stylist}\n`;
  if (date) waText += `📅 *Date:* ${date}\n`;
  if (time) waText += `⏰ *Time Slot:* ${time}\n`;
  if (price) waText += `💰 *Price:* AED ${price}\n`;
  if (notes) waText += `📝 *Notes:* ${notes}\n`;
  waText += `\n📍 *Studio:* Dubai, United Arab Emirates\n`;
  waText += `💬 _Please reply to confirm availability and schedule._`;

  const encoded = encodeURIComponent(waText);
  const waUrl = `https://wa.me/${WHATSAPP_NUMBER}?text=${encoded}`;

  // Update modal content
  const modal = document.getElementById('waBookingSuccessModal');
  const detailsEl = document.getElementById('waSummaryDetails');
  const sendBtn = document.getElementById('waDirectSendBtn');
  const overlay = document.getElementById('waBookingSuccessOverlay');
  const closeBtn = document.getElementById('closeWaSuccessModalBtn');

  if (detailsEl) {
    detailsEl.innerHTML = `
      <div><strong>Client:</strong> ${name} (${phone})</div>
      <div><strong>Service:</strong> ${service || type || 'Consultation / Demo'}</div>
      ${stylist ? `<div><strong>Selected Specialist:</strong> <span style="color:var(--gold-light);">${stylist}</span></div>` : ''}
      ${date ? `<div><strong>Date & Time:</strong> ${date} at ${time}</div>` : ''}
    `;
  }
  ...
  // Attempt auto-opening WhatsApp
  try {
    const w = window.open(waUrl, '_blank');
    if (!w || w.closed || typeof w.closed === 'undefined') {
      console.log('Popup prevented; accessible via confirmation modal button');
    }
  } catch (e) {
    console.warn('WhatsApp auto-open exception:', e);
  }
}

// Fetch active stylists from backend and populate dropdowns
async function populateGlobalArtistDropdowns() {
  try {
    const res = await fetch(`${API_BASE_URL}/api/stylists`);
    if (res.ok) {
      const stylists = await res.json();
      if (Array.isArray(stylists) && stylists.length > 0) {
        document.querySelectorAll('.artist-selector-dropdown').forEach(select => {
          const currentVal = select.value;
          select.innerHTML = '<option value="">Any Available Senior Specialist</option>' +
            stylists.map(s => `<option value="${s}">${s}</option>`).join('');
          if (currentVal) select.value = currentVal;
        });
      }
    }
  } catch (e) {
    console.warn('Could not load stylists for dropdown:', e);
  }
}
...
function initContactForm() {
  ...
  if (form) {
    form.addEventListener("submit", async (e) => {
      ...
      const consultStylistEl = document.getElementById("consultStylist");
      const stylist = consultStylistEl ? consultStylistEl.value : "";
      const name = document.getElementById("name").value.trim();
      const phone = document.getElementById("phone").value.trim();
      const email = document.getElementById("email") ? document.getElementById("email").value.trim() : "";
      const hairloss = document.getElementById("hairloss") ? document.getElementById("hairloss").value : "";
      ...
      try {
        ...
        // Also create a consultation booking in /api/bookings so the artist portal receives it
        if (stylist) {
          try {
            await fetch(`${API_BASE_URL}/api/bookings`, {
              ...
              body: JSON.stringify({
                name: name,
                phone: phone,
                service: "Free Demo Consultation",
                bookingType: "Consultation",
                stylist: stylist,
                date: new Date().toISOString().split('T')[0],
                time: "Flexible / TBD",
                notes: message || "Free Consultation Request via website",
                status: "Pending"
              })
            });
          } catch (bErr) {}
        ...
        showToast("Consultation request submitted! Opening WhatsApp confirmation...", "success");
        triggerWhatsAppBookingNotification({
          type: "Free Demo Consultation",
          name: name,
          phone: phone,
          service: "Free Demo Consultation & Hair Evaluation",
          stylist: stylist,
          notes: message
        });
        form.reset();
        ...
        showToast("Request recorded! Opening WhatsApp...", "success");
        triggerWhatsAppBookingNotification({
          type: "Free Demo Consultation",
          name: name,
          phone: phone,
...
// Helper to normalize time strings to canonical 24-hour HH:MM format
...
// Fetch booked slots for a given date from API
async function fetchBookedSlots(dateStr) {
  if (!dateStr) return [];
  try {
    const res = await fetch(`${API_BASE_URL}/api/bookings/booked-slots?date=${encodeURIComponent(dateStr)}`);
    if (res.ok) {
      const data = await res.json();
      return data.bookedSlots24 || [];
    }
  } catch (err) {
    console.warn('Could not fetch from /api/bookings/booked-slots, trying fallback:', err);
  }

  // Graceful fallback to /api/bookings if needed
  try {
    ...
    if (res.ok) {
      ...
      return (bookings || [])
        ...
        .map(b => normalizeTimeTo24h(b.time))
        .filter(Boolean);
    }
  } catch (e) {
...
function initMaintenanceForm() {
  ...
  const timeSelect = document.getElementById('maintTime');

  const ALL_MAINTENANCE_SLOTS = [
    ...
    { value: "05:30 PM", label: "05:30 PM - Evening" },
    { value: "07:00 PM", label: "07:00 PM - Evening" },
    { value: "08:30 PM", label: "08:30 PM - Evening" }
  ];

  async function updateMaintenanceSlots(selectedDate) {
    ...
    timeSelect.disabled = true;
    timeSelect.innerHTML = '<option value="">Checking available slots...</option>';

    const booked24 = await fetchBookedSlots(selectedDate);

    // If today is selected, filter out past slots as well
    const today = new Date();
    const todayStr = today.toISOString().split('T')[0];
    const isToday = (selectedDate === todayStr);
    const nowMinutes = today.getHours() * 60 + today.getMinutes();

    // Filter out already booked and past slots
    const availableSlots = ALL_MAINTENANCE_SLOTS.filter(slot => {
      const slot24 = normalizeTimeTo24h(slot.value);
      if (booked24.includes(slot24)) {
        return false; // Booked: do NOT show this slot
      }
      if (isToday) {
    ...
    if (availableSlots.length === 0) {
      timeSelect.innerHTML = '<option value="" disabled selected>No slots available for this date — please choose another date</option>';
      timeSelect.value = '';
    } else {
      let optionsHtml = `<option value="">Select Time Slot (${availableSlots.length} available)</option>`;
      availableSlots.forEach(slot => {
        optionsHtml += `<option value="${slot.value}">${slot.label}</option>`;
      });
      timeSelect.innerHTML = optionsHtml;

      // Retain previously selected slot if it is still available
      const stillAvailable = availableSlots.some(s => s.value === previousValue);
      if (stillAvailable) {
  ...
  if (dateInput) {
    ...
    dateInput.addEventListener('change', (e) => {
      updateMaintenanceSlots(e.target.value);
    });
    ...
    if (dateInput.value) {
      updateMaintenanceSlots(dateInput.value);
    } else if (timeSelect) {
      timeSelect.innerHTML = '<option value="">Select Date First</option>';
      timeSelect.disabled = true;
    }
  ...
  if (maintForm) {
    maintForm.addEventListener('submit', async (e) => {
      ...
      const submitBtn = document.getElementById('maintSubmitBtn') || maintForm.querySelector('button[type="submit"]');
      const originalBtnText = submitBtn ? submitBtn.innerHTML : 'Confirm Maintenance Booking';

      const name = document.getElementById('maintName').value.trim();
      const phone = document.getElementById('maintPhone').value.trim();
      const serviceSelect = document.getElementById('maintService');
      const service = serviceSelect.value;
      const selectedOpt = serviceSelect.options[serviceSelect.selectedIndex];
      const price = selectedOpt ? Number(selectedOpt.dataset.price || 199) : 199;
      const duration = selectedOpt ? (selectedOpt.dataset.duration || '45 mins') : '45 mins';
      const maintStylistEl = document.getElementById('maintStylist');
      const stylist = maintStylistEl ? maintStylistEl.value : '';
      ...
      if (submitBtn) {
        ...
        submitBtn.innerHTML = `
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="animation: spin 1s linear infinite; display: inline-block; vertical-align: middle; margin-right: 6px;">
            <circle cx="12" cy="12" r="10" stroke-opacity="0.25"></circle>
            <path d="M12 2a10 10 0 0 1 10 10" stroke-linecap="round"></path>
          </svg>
          Reserving Maintenance Slot...
        `;
      }
      ...
      const saveBookingLocally = (bookingItem) => {
        try {
          const list = JSON.parse(localStorage.getItem('cs_bookings') || '[]');
          const idx = list.findIndex(b => b.id === bookingItem.id);
          if (idx !== -1) {
            list[idx] = bookingItem;
          } else {
            list.unshift(bookingItem);
          }
          localStorage.setItem('cs_bookings', JSON.stringify(list));
        } catch (e) {
          console.warn('LocalStorage save error:', e);
        }
      };

      // Create fallback booking record in localStorage immediately
      const tempId = 'b_' + Date.now().toString(36) + Math.random().toString(36).substring(2, 5);
      const localBookingRecord = {
        id: tempId,
        ...data,
        timestamp: new Date().toISOString()
      };
      saveBookingLocally(localBookingRecord);
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (resData && resData.booking) {
            saveBookingLocally(resData.booking);
          }

          showToast('Maintenance booking confirmed! Opening WhatsApp...', 'success');
          triggerWhatsAppBookingNotification({
          ...
          if (date) updateMaintenanceSlots(date);
        } else {
          const errData = await response.json().catch(() => ({}));
          if (response.status === 409) {
            showToast(errData.error || 'This slot is already booked. Please select another time.', 'error');
            if (date) updateMaintenanceSlots(date);
          } else {
            showToast('Maintenance booking recorded in dashboard! Opening WhatsApp...', 'success');
            triggerWhatsAppBookingNotification({
              type: 'Maintenance Session',
              name,
              phone,
        ...
        console.warn('Server offline/unreachable; booking safely recorded locally:', err);
        showToast('Maintenance booking confirmed! Opening WhatsApp...', 'success');
        triggerWhatsAppBookingNotification({
          type: 'Maintenance Session',
          name,
...
function initDirectServiceModal() {
  ...
  const slotBtns = modal ? modal.querySelectorAll('.ds-slot-btn') : [];
  const selectedTimeInput = document.getElementById('dsSelectedTime');

  if (!modal) return;

  async function updateDirectServiceSlots(selectedDate) {
    if (!selectedDate || !slotBtns.length) return;
    const booked24 = await fetchBookedSlots(selectedDate);
    ...
    slotBtns.forEach(btn => {
      ...
      const isBooked = booked24.includes(time24);
      let isPassed = false;
      if (isToday) {
        const [h, m] = time24.split(':').map(Number);
        if ((h * 60 + m) <= nowMinutes) isPassed = true;
      }

      if (isBooked || isPassed) {
        btn.style.display = 'none'; // Not shown because already booked/passed
        btn.disabled = true;
        btn.classList.remove('active');
        btn.classList.add('ds-slot-booked');
      } else {
        btn.style.display = 'flex';
        btn.disabled = false;
        btn.classList.remove('ds-slot-booked');
    ...
    // Check if the currently active slot is still visible and available
    const activeBtn = modal.querySelector('.ds-slot-btn.active');
    if (!activeBtn || activeBtn.style.display === 'none' || activeBtn.disabled) {
      if (firstAvailableBtn) {
        slotBtns.forEach(s => s.classList.remove('active'));
        firstAvailableBtn.classList.add('active');
        if (selectedTimeInput) selectedTimeInput.value = firstAvailableBtn.dataset.time;
      } else {
  ...
  if (dateInput) {
    ...
    dateInput.addEventListener('change', (e) => updateDirectServiceSlots(e.target.value));
    dateInput.addEventListener('input', (e) => updateDirectServiceSlots(e.target.value));
  }
  ...
  function openModal() {
    ...
    if (dateInput && dateInput.value) {
      updateDirectServiceSlots(dateInput.value);
    }
  }
  ...
  slotBtns.forEach(slot => {
    slot.addEventListener('click', () => {
      if (slot.disabled || slot.style.display === 'none' || slot.classList.contains('ds-slot-booked')) return;
      slotBtns.forEach(s => s.classList.remove('active'));
  ...
  if (form) {
    form.addEventListener('submit', async (e) => {
      ...
      const dsStylistEl = document.getElementById('dsStylist');
      const stylist = dsStylistEl ? dsStylistEl.value : '';
      ...
      const bookingData = {
        ...
        price: Number(price),
        duration,
        bookingType: stylist ? 'Partner' : 'Direct Booking',
        status: 'Confirmed',
      ...
      try {
        ...
        if (response.ok) {
          showToast('Appointment Reserved! Opening WhatsApp confirmation...', 'success');
          triggerWhatsAppBookingNotification({
            type: 'Direct Studio Booking',
            name,
            phone,
            service,
            price,
          ...
          if (date) updateDirectServiceSlots(date);
        }
      } catch (err) {
