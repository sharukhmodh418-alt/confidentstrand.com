    ...
    maintForm.addEventListener('submit', async (e) => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          showToast('Maintenance booking confirmed! Your appointment has been sent to the dashboard.', 'success');
          maintForm.reset();
          if (timeSelect) {
            timeSelect.innerHTML = '<option value="">Select Date First</option>';
            timeSelect.disabled = true;
          ...
          if (response.status === 409) {
            ...
            // Still confirm local capture so customer is not blocked
            showToast('Maintenance booking recorded in dashboard!', 'success');
            maintForm.reset();
            if (timeSelect) {
              timeSelect.innerHTML = '<option value="">Select Date First</option>';
              timeSelect.disabled = true;
        ...
        showToast('Maintenance booking confirmed! Recorded in dashboard.', 'success');
        maintForm.reset();
        if (timeSelect) {
          timeSelect.innerHTML = '<option value="">Select Date First</option>';
          timeSelect.disabled = true;
